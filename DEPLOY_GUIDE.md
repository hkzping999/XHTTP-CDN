...
高级 URI 包含 `encryption=<PQC>`、`ech=<ECH>`、`type=xhttp`、`mode`、`host`、`path` 与 `extra=<URL-encoded JSON>`。开启上下行分离时，`extra.downloadSettings` 内含独立下行 `address/port`、`stream-down`、TLS SNI/ALPN 和 ECH。
