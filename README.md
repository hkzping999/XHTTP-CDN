...
如果上行模式原来是 `stream-one`，脚本会自动改为 `stream-up`，因为 Xray 的 `stream-one` 不能与 `downloadSettings` 同时使用。默认的 `packet-up` 可以直接配独立 `stream-down`。引导安装 `argox -g` 和安装后的“修改配置”菜单都提供同一开关。
